...
const AccountProfile = ({ user, btnTitle }: Props) => {
  ...
  const pathname = usePathname();
    ...
    const handleImage = (e: ChangeEvent<HTMLInputElement>, fieldChange: (value: string) => void) => {
      ...
      const fileReader = new FileReader(); // initalise a new file reader
    ...
    const onSubmit = async (values: z.infer<typeof UserValidation>) => {
      ...
      const hasImageChanged = isBase64Image(blob); // check if the user has changed the image
      ...
      await updateUser({ // makes code less error prone because we are only passing in one object
        name: values.name,
        path: pathname,
